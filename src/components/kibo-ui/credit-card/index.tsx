...
export const CreditCardChip = ({ className, children, ...props }: CreditCardChipProps) =>
  children ? (
    <div className={cn('absolute top-1/2 left-0 w-1/6 shrink-0 -translate-y-1/2 rounded-[18%]', className)}>
      {children}
    </div>
  ) : (
    <svg
...
export const CreditCardMagStripe = ({ className, ...props }: CreditCardMagStripeProps) => {
  ...
  return (
    <div
      className={cn('absolute top-[3%] left-1/2 h-1/4 -translate-x-1/2 bg-gray-900', className)}
      style={{
